...
{
    [RoutePrefix("api/internal/settings")]
    public class InternalSettingDefaultEndpointController : ApiController
    {
        ...
        public InternalSettingDefaultEndpointController()
        {
            m_entityName = nameof(Setting);
            m_entityRoute = "settings";
            ...
            [FromUri(Name = "size")]int size = 10)
        {
            ...
            foreach (var jtok in json)
            {
                ...
                var item = jtok.Value.ToObject<Setting>();
        ...
        [HttpPut]
        [Route("{id}")]
        public async Task<HttpResponseMessage> PutDefault([FromBody]Setting item, string id)
        {
            var resultSuccess = true;
            var resultStatus = "OK";
            ...
            var output = await response.Content.ReadAsStringAsync();
            item = JObject.Parse(output).ToObject<Setting>();
            item.FirebaseKey = id;
        ...
        private async Task<Setting> PrivateGetOneByIdAsync(string id)
        {
