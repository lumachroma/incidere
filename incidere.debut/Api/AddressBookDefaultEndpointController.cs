using incidere.debut.Models.AddressBook;
...
{
    [RoutePrefix("api/address-books")]
    public class AddressBookDefaultEndpointController : ApiController
    {
        ...
        public AddressBookDefaultEndpointController()
        {
            m_entityName = nameof(AddressBook);
            m_entityRoute = "address-books";
            ...
            [FromUri(Name = "size")]int size = 10)
        {
            ...
            foreach (var jtok in json)
            {
                ...
                var item = jtok.Value.ToObject<AddressBook>();
        ...
        public async Task<HttpResponseMessage> GetOneAsync(string id)
        {
            ...
            var item = json.ToObject<AddressBook>();
            item.FirebaseKey = id;
        ...
        [HttpPost]
        [Route("")]
        public async Task<HttpResponseMessage> PostDefault([FromBody]AddressBook item)
        {
            var resultSuccess = true;
            var resultStatus = "OK";
        ...
        [HttpPut]
        [Route("{id}")]
        public async Task<HttpResponseMessage> PutDefault([FromBody]AddressBook item, string id)
        {
            var resultSuccess = true;
            var resultStatus = "OK";
            ...
            var output = await response.Content.ReadAsStringAsync();
            item = JObject.Parse(output).ToObject<AddressBook>();
            item.FirebaseKey = id;
        ...
        private async Task<AddressBook> PrivateGetOneByIdAsync(string id)
        {
                ...
                try
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        try
                        {
                            ...
                            if (null != json)
                            {
                                item = json.ToObject<AddressBook>();
                                item.FirebaseKey = id;
                            }
                        }
                        catch (Exception)
